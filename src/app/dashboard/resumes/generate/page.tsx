...
import { ArrowLeft, FileText, Download, Eye, Loader2, AlertCircle } from "lucide-react";
import Link from "next/link";
import { useResumeData, useResumeGeneration } from "@/hooks/use-resume-generation";
...
export default function GenerateResumePage() {
  ...
  const { data: resumeData, isLoading: dataLoading, error: dataError } = useResumeData();
  const { generateResume, generatePreview, isValidating, isGenerating, isGeneratingPreview, validationError, generationError, previewError } = useResumeGeneration();
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">Generate Resume</h1>
            <p className="text-muted-foreground text-red-600">Error loading profile data</p>
          </div>
        </div>
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center space-x-2 text-red-600">
              ...
              <p>Failed to load profile data. Please try again or check your profile setup.</p>
            </div>
          </CardContent>
        </Card>
      </div>
  ...
  const hasMinimalData = resumeData?.profile && (
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Customize your resume title and select which sections to include
                </CardDescription>
              </CardHeader>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setValue("contentSelection.includePersonalInfo", !!checked)}
                      />
                      <Label htmlFor="includePersonalInfo">Personal Information</Label>
                      {resumeData?.profile && (
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setValue("contentSelection.includeSummary", !!checked)}
                      />
                      <Label htmlFor="includeSummary">Professional Summary</Label>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setValue("contentSelection.includeWorkExperience", !!checked)}
                      />
                      <Label htmlFor="includeWorkExperience">Work Experience</Label>
                      {resumeData?.workExperiences && resumeData.workExperiences.length > 0 && (
                        <Badge variant="secondary">{resumeData.workExperiences.length} items</Badge>
                      )}
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setValue("contentSelection.includeEducation", !!checked)}
                      />
                      <Label htmlFor="includeEducation">Education</Label>
                      {resumeData?.education && resumeData.education.length > 0 && (
                        <Badge variant="secondary">{resumeData.education.length} items</Badge>
                      )}
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setValue("contentSelection.includeSkills", !!checked)}
                      />
                      <Label htmlFor="includeSkills">Skills</Label>
                      {resumeData?.skills && resumeData.skills.length > 0 && (
                        <Badge variant="secondary">{resumeData.skills.length} skills</Badge>
                      )}
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setValue("contentSelection.includeProjects", !!checked)}
                      />
                      <Label htmlFor="includeProjects">Projects</Label>
                      {resumeData?.projects && resumeData.projects.length > 0 && (
                        <Badge variant="secondary">{resumeData.projects.length} projects</Badge>
                      )}
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setValue("contentSelection.includeCertifications", !!checked)}
                      />
                      <Label htmlFor="includeCertifications">Certifications</Label>
                      {resumeData?.certifications && resumeData.certifications.length > 0 && (
                        <Badge variant="secondary">{resumeData.certifications.length} certs</Badge>
                      )}
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setValue("contentSelection.includeAchievements", !!checked)}
                      />
                      <Label htmlFor="includeAchievements">Achievements</Label>
                      {resumeData?.achievements && resumeData.achievements.length > 0 && (
                        <Badge variant="secondary">{resumeData.achievements.length} achievements</Badge>
                      )}
                    </div>
                  </div>
                </div>
